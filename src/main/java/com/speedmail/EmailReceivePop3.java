...
public class EmailReceivePop3 {
    ...
    public static void receive(EmailUserInfo emailUser, List<EmailReceiveEmailInfo> listReceiveEmailinfo, List<MimeMessage> listReceiveEmailMsg,EmailDbOperate emailDbOperate) throws MessagingException, IOException {
        ...
        for (int i = 0, count = messages.length; i < count; i++) {
            ...
            emailReceiveEmailInfo.setSendDate(getSentDate(msg, "yyyy-MM-dd hh:mm:ss"));
...
//            }
            ...
            if (subject_o.indexOf("POSTMASTER") < 0
                    && subject_o.indexOf("MAIL DELIVERY SYSTEM") < 0 && subject_o.indexOf("STUDY@SERVICE.NETEASE.COM") < 0
                    && subject_o.indexOf("51JOB@SERVICE.NETEASE.COM") < 0 && subject_o.indexOf("MAIL@SERVICE.NETEASE.COM") < 0
                    && subject_o.indexOf("LMLC@SERVICE.NETEASE.COM") < 0 && subject_o.indexOf("YOUQIAN@SERVICE.NETEASE.COM") < 0
                    && subject_o.indexOf("YOUQIAN@SERVICE.NETEASE.COM") < 0 && subject_o.indexOf("MAIL DELIVERY SUBSYSTEM") < 0
                    && subject_o.indexOf("FA163@SERVICE.NETEASE.COM") < 0 && subject_o.indexOf("MAILCLOUD DELIVER SYSTEM") < 0
                    && subject_o.indexOf("AUTOMATIC REPLY") < 0 && subject_o.indexOf("AUTO") < 0
                    && subject_o.indexOf("AUTOMATISCH ANTWOORD") < 0 && subject_o.indexOf("AUTOSVAR") < 0
                    && subject_o.indexOf("UNDELIVERABLE") < 0 && subject_o.indexOf("系统退信") < 0) {
